...
bool check(char* word)
{
    // Get len of the word (needed to traverse it)
    int word_len = strlen(word);
    // Set cursor to root node pointer
    node *cursor = root;
    // Iterate over chars of our word
    for (int i = 0; i < word_len; i++)
    {
        char letter = word[i];
        // Handle letter case, we convert all chars to lower
        if (!islower(letter))
        {
            letter = tolower(letter);
        }
        // ASCII conversion, we convert our letter to an index
        int ind = (int) letter - 97;
        // Check chlidren index for NULL
        if (cursor -> children[ind] == NULL)
        {
            return false;
        }
        // Re-point our cursor
        cursor = (cursor -> children[ind]);
        // Check if it is our last letter & look at is_word
        if (i == (word_len - 1))
        {
